...
/** animateViewControllerVC */
@property (nonatomic, strong) FLHAnimateViewController *animateViewControllerVC;
...
- (void)moveViewWithGesture:(UIPanGestureRecognizer *)panGes {
    if (panGes.state == UIGestureRecognizerStateEnded) {
        if ([self.childViewControllers containsObject:self.animateViewControllerVC]) return;
        [self profileCenter];
    }
}

- (void)msgClick {
    FLHSearchViewController *vc = [[FLHSearchViewController alloc] init];
...
- (void)profileCenter {
    ...
    // 展示个人中心
    FLHAnimateViewController *vc = [[FLHAnimateViewController alloc] init];
    self.animateViewControllerVC = vc;
